...
export default {
  ...
  methods: {
    ...
    getRoomUrl(isTestRoom) {
      ...
      let query = {...this.form}
      delete query.roomId
      query.imgTransformer = window.localStorage.imgTransformer == null ? '[{"from":"(轴依的肯定)","target":"JOIYES.jpg"},{"from":"(轴依的的否定)","target":"JOINO.jpg"}]' : window.localStorage.imgTransformer
      let resolved
      if (isTestRoom) {
